...
public class MainActivity extends BaseActivity
    ...
    private void initNavigationView() {
        ...
        menu.getItem(0).setIcon(    //新闻
                new IconicsDrawable(this).
                        icon(GoogleMaterial.Icon.gmd_explore));
        menu.getItem(1).setIcon(    //妹子
                new IconicsDrawable(this).
                        icon(GoogleMaterial.Icon.gmd_face)
                        .color(Color.RED));
        menu.findItem(R.id.nav_weibo).setIcon(    //微博
                new IconicsDrawable(this).
                        icon(GoogleMaterial.Icon.gmd_account_balance)
                        .color(Color.RED));
        menu.findItem(R.id.nav_secret_mode).setIcon(    //secret
                new IconicsDrawable(this).
                        icon(GoogleMaterial.Icon.gmd_ac_unit)
                        .color(Color.RED));
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        int id = item.getItemId();
        if (id == R.id.nav_knowledge) { //知识
            replace(TabsFragment.MENU_NEWS);
        } else if (id == R.id.nav_beauty) { //妹子
            replace(TabsFragment.MENU_PIC);
        } else if (id == R.id.nav_weibo) {  //微博
            replace(TabsFragment.MENU_WEIBO);
        }else if (id == R.id.nav_secret_mode) { //secret
